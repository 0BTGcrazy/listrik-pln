...
// Ambil data petugas terakhir berdasarkan tanggal
$petugas = $aksi->caridata("petugas WHERE id_petugas LIKE '%$today%' ORDER BY id_petugas DESC");

// Cek apakah ada data
if ($petugas && isset($petugas['id_petugas'])) {
    // Ambil 3 digit terakhir dari ID (urutan angka)
    $kode = (int)substr($petugas['id_petugas'], 9, 3) + 1;
} else {
    // Jika belum ada data hari ini, mulai dari 1
    $kode = 1;
}

// Buat ID petugas baru, format: PYYYYMMDDXXX
$id_petugas = sprintf("P%s%03d", $today, $kode);
